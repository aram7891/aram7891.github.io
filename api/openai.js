...
// --- 2. MOTOR OPENAI (estable, flexible y sin errores tontos) ---
export default async function handler(req, res) {
  try {
    const { texto, tipo } = req.body;

    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) throw new Error("API_KEY_MISSING: Falta OPENAI_API_KEY en Vercel.");
    ...
    console.error("--- ERROR EN OPENAI.JS ---");
